...
class Explosion:
    def __init__(self, x_cord: int, y_cord: int, choose_explosion: int):
        self.image = [pygame.image.load(f'explosion/explosion_{x}_6.png').convert_alpha() for x in range(0, 7)]
        self.big_image = [pygame.image.load(f'explosion/big_explosion_{x}_6.png').convert_alpha() for x in range(0, 7)]
        self.x_cord = x_cord
        self.y_cord = y_cord
        self.choose_image = 0
        self.counter = 0
